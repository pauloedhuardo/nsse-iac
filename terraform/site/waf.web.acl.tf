resource "aws_wafv2_web_acl" "this" {
  name  = var.waf_webacl.name
  scope = var.waf_webacl.scope
  ...
  custom_response_body {
    key = var.waf_webacl.custom_response_body.key
    content = jsonencode({
      message = var.waf_webacl.custom_response_body.message
    })
    content_type = var.waf_webacl.custom_response_body.content_type
  }

  visibility_config {
    cloudwatch_metrics_enabled = var.waf_webacl.visibility_config.cloudwatch_metrics_enabled
    metric_name                = var.waf_webacl.visibility_config.metric_name
    sampled_requests_enabled   = var.waf_webacl.visibility_config.sampled_requests_enabled
  }
